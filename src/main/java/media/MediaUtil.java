...
public class MediaUtil {
     ...
     * @param outputFile 转换后的视频路径
     * @param startTime  开始抽取的时间点（单位:s）
     * @param endTime    结束抽取的时间点
     */
    public static void cutVideo(File videoFile, File outputFile, Time startTime, Time endTime) {
        if (videoFile == null || !videoFile.exists()) {
            throw new RuntimeException("视频文件不存在：");
        }
        if (null == outputFile) {
     ...
     * https://blog.csdn.net/minato725/article/details/107071494/
     */
    public static void concatVideoList(List<String> videoPathList,
                                       @NotNull ConcatType type,
                                       boolean isDeleteOriginFile) throws IOException {
        switch (type) {
            case DEMUXER:
                // 相同格式时使用 Concat demuxer
                concatWithDemuxer(videoPathList, isDeleteOriginFile);
            case FILTER:
                //TODO 混合格式使用 Concat filter
        }
    }
    ...
    private static void concatWithDemuxer(List<String> videoPathList, boolean isDeleteOriginFile) throws IOException {
        ...
        String txtPath = mergedFilePath + ".txt";
        ...
        command.add(txtPath);
        command.add("-c");
        command.add("copy");// -c copy 避免解码，提高处理速度
        command.add(mergedFilePath);
